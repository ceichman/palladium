...
class MatrixUtils {
    ...
    static let WEST = PITCHAXIS
    static let UP = YAWAXIS
    static let NORTH = ROLLAXIS
    static let LEFT = WEST
    static let FORWARD = NORTH
    
    static func matrixPointAt(pos: simd_float3, target: simd_float3) -> float4x4 {
    ...
    static func projectionMatrix(aspectRatio: Float, fovRadians: Float, nearZ: Float, farZ: Float) -> simd_float4x4 {
    ...
    static func translationMatrix(t: simd_float3) -> simd_float4x4 {
    ...
    static func rotationMatrix(axis: vector_float3, theta: Float) -> matrix_float4x4
    {
        let c = cos(theta);
        let s = sin(theta);
    ...
    static func scalingMatrix(scale: simd_float3) -> simd_float4x4 {
    ...
    static func magnitude(_ vec: simd_float3) -> Float {
        return sqrt(vec.x * vec.x + vec.y * vec.y + vec.z * vec.z);
    }
    
    static func createModelTransform(position: simd_float3, eulerRotation: simd_float3, scale: simd_float3) -> ModelTransform {
        let translationMatrix = MatrixUtils.translationMatrix(t: position)
        let rotationMatrix =
            rotationMatrix(axis: PITCHAXIS, theta: eulerRotation.x) *
            rotationMatrix(axis: YAWAXIS, theta: eulerRotation.y) *
            rotationMatrix(axis: ROLLAXIS, theta: eulerRotation.z)
        let scalingMatrix = scalingMatrix(scale: scale)
