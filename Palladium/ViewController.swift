...
class ViewController: UIViewController, MTKViewDelegate {
    ...
    func render() {
        ...
        var transformationParams = TransformationParams(origin: simd_float3(0.0, -2.0, 6.0), rotation: simd_float3(xPosition, yPosition, 0), scale: simd_float3(1, 1, 1))
    ...
    func draw(in view: MTKView) {
        autoreleasepool {
            ...
            let aspectRatio = drawable.texture.height / drawable.texture.width
            let fovDegrees = 40.0
            var projectionParams = ProjectionParams(aspectRatio: Float(aspectRatio),
                                                    fovRadians: Float(fovDegrees / 180.0 * Double.pi),
                                                    nearZ: 0.3,
